...
- 已移除“跳过”功能：
  - 删除顶部跳过按钮
  - 删除 `S` 快捷键提示与事件
  - 删除相关状态字段与逻辑分支（`isSkip` / `toggleSkip`）
- 优化人物与 UI 遮挡：
  - 新增顶部保留空间变量 `--top-ui-reserve`
  - 立绘舞台改为“顶部 UI + 底部对话框”双向动态避让
  - `syncPortraitLayout()` 同步计算顶部和底部占用高度
- 手机端优化：
  - 顶部功能区改为可横向滚动单行，减少高度挤压
  - 统计条与按钮尺寸微调
  - 立绘最大高度在小屏下按可视空间自动压缩

2026-03-27（剧情重写：前辈与学妹）
- 主线改为“奶龙前辈与蒝蒝学妹”校园恋爱剧情：
  - 篮球课相遇 + 球飞来被单手接住的开场
  - 训练互动与攻略推进
  - 校园跑切歌跑错路线，多跑 2km 的情节
  - 奶龙前辈安慰学妹的关键节点
- 结局图鉴文案更新为校园恋爱主题。
